...
// updated UserController.js to include the deleteUser function.
...
const postRegister = async (req, res, next) => {
  try {
    ...
    const token = jwt.sign(
      ...
      process.env.JWT_SECRET_KEY,
      { expiresIn: "1h" }
    ); // user data saved in sessionStorage will expire in 1 hour
    await sendConfirmationEmail(email, token);
...
/*import { insertUser, getUser, updateUserStatus, deleteUser, deleteUserReviews, deleteUserFavorites, deleteUserSharedFavorites } from "../models/User.js";
import { compare, hash } from "bcrypt";
import { ApiError } from "../helpers/ApiError.js";
import { sendConfirmationEmail } from "../helpers/emailService.js";
import jwt from "jsonwebtoken";
...
const deleteAccount = async (req, res, next) => {
  try {
    ...
    await deleteUserSharedFavorites(userId);
...
export { postRegister, postLogin, confirmUser, deleteAccount };*/
